                    ...
                    <div class="account__content">
                        ...
                        <form class="contact__form--inner" action="/admin/update_product?id=<?php echo $product['id']; ?>" method="post" enctype="multipart/form-data">
                            <div class="row">
                                ...
                                <!-- Thêm option vào form -->
                                <label class="contact__form--label">Chọn ảnh:</label>
                                <select name="image_option" onchange="showImageModal()">
                                    <option value="keep">Giữ lại ảnh cũ</option>
                                    <option value="new">Tải lên ảnh mới</option>
                                    <option value="existing">Chọn ảnh từ folder uploads</option>
                                </select>

                                <!-- Thêm trường ẩn để lưu tên ảnh được chọn từ modal -->
...
<!-- Thêm modal vào form của bạn -->
<div id="imageModal" class="modal">
    <div class="modal-content">
        <span class="close" onclick="closeImageModal()">&times;</span>
        <h2>Chọn ảnh từ folder uploads</h2>
        <?php
            // Hiển thị danh sách ảnh từ folder uploads
            $imageDir = "public/uploads/";
            $images = scandir($imageDir);
            foreach ($images as $image) {
                if ($image != "." && $image != "..") {
                    echo "<img src='$imageDir$image' onclick='selectImage(\"$image\")' alt='Uploaded Image'>";
                }
            }
        ?>
    </div>
</div>
<!-- Thêm mã JavaScript vào trong trang HTML của bạn -->
<script>
    // Hiển thị modal khi chọn option thứ hai
