...
class User
{
    ...
    public function adminUpdateUser($id, $name, $first, $quantity, $image, $shortDesc, $longDesc)
    {
        $conn = $this->getConnection();
        $query = "UPDATE ps_users 
              SET name = :name, first = :first, quantity = :quantity, image = :image, short_desc = :short_desc, long_desc = :long_desc
              WHERE id = :id";
        $stmt = $conn->prepare($query);
