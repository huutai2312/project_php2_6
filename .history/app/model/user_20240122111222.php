...
class User
{
    ...
    public function adminUpdateUser($id, $name, $fi, $quantity, $image, $shortDesc, $longDesc)
    {
        $conn = $this->getConnection();
        $query = "UPDATE ps_users 
              SET name = :name, fi = :fi, quantity = :quantity, image = :image, short_desc = :short_desc, long_desc = :long_desc
              WHERE id = :id";
        $stmt = $conn->prepare($query);
        $stmt->bindValue(':id', $id, PDO::PARAM_INT);
        $stmt->bindValue(':name', $name, PDO::PARAM_STR);
        $stmt->bindValue(':fi', $fi, PDO::PARAM_STR);
