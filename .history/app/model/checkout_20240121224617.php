...
class Checkout
{
    ...
    public function saveOrder($email, $first_name, $last_name, $company, $address, $phone, $city, $country, $postal_code, $total_price)
    {
        try {
            ...
            $stmt = $conn->prepare("INSERT INTO ps_order (email, first_name, last_name, company, address, phone, city, country, postal_code, total_price, status, time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$email, $first_name, $last_name, $company, $address, $phone, $city, $country, $postal_code, $total_price, $status, $time]);
