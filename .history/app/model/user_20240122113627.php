...
class User
{
    ...
    public function adminUpdateUser($id, $name, $first_name, $last_name, $age, $company, $email, $password, $address, $address2, $phone, $city, $country, $postal_code, $is_admin)
    {
        ...
        $stmt->bind(':name', $name, PDO::PARAM_STR);
        $stmt->bind(':first_name', $first_name, PDO::PARAM_STR);
        $stmt->bind(':last_name', $last_name, PDO::PARAM_INT);
        $stmt->bind(':age', $age, PDO::PARAM_STR);
        $stmt->bind(':company', $company, PDO::PARAM_STR);
        $stmt->bind(':email', $email, PDO::PARAM_STR);
        $stmt->bind(':password', $password, PDO::PARAM_STR);
        $stmt->bind(':address', $address, PDO::PARAM_STR);
        $stmt->bind(':address2', $address2, PDO::PARAM_STR);
        $stmt->bind(':phone', $phone, PDO::PARAM_STR);
        $stmt->bind(':city', $city, PDO::PARAM_STR);
        $stmt->bind(':country', $country, PDO::PARAM_STR);
        $stmt->bind(':postal_code', $postal_code, PDO::PARAM_STR);
        $stmt->bind(':is_admin', $is_admin, PDO::PARAM_STR);
