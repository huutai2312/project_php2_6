...
class User
{
    ...
    public function registerUser($name, $email, $password, $date)
    {
        $conn = $this->getConnection();

        $query = "INSERT INTO ps_user (name, email, password, address, phone, is_admin, date_created) VALUES (:name, :email, :password, :address, :phone, :is_admin, NOW())";
        $stmt = $conn->prepare($query);
        ...
        $stmt->bindValue(':date_created', NOW(), PDO::PARAM_STR);
