    ...
    public function addtocart()
{
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Kiểm tra xem user đã nhấn nút cập nhật giỏ hàng hay chưa
        if (isset($_POST['updateCart'])) {
            // Lấy giá trị mới của số lượng từ form
            $newQuantities = $_POST['quantity'];
            $productIds = $_POST['product_id'];

            // Cập nhật giỏ hàng theo giá trị mới
            foreach ($_SESSION['cart'] as &$cartItem) {
                $index = array_search($cartItem[0], $productIds);
                if ($index !== false) {
                    $cartItem[1] = $newQuantities[$index];
                }
            }
        } else {
            // Nếu không phải là nút cập nhật, thực hiện thêm mới vào giỏ hàng
