    ...
    public function adminUpdateProduct()
{
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($_POST['image_option']) && $_POST['image_option'] == 'new') {
            // Người dùng chọn tải lên ảnh mới
            if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
                $image = $_FILES['image']['name'];
                $targetDir = "public/uploads/";
                $targetFile = $targetDir . basename($_FILES['image']['name']);
        ...
        } elseif (isset($_POST['image_option']) && $_POST['image_option'] == 'existing') {
            // Người dùng chọn ảnh từ folder uploads
            if (isset($_POST['selected_image'])) {
                $image = $_POST['selected_image'];
            }
        } else {
            // Người dùng không chọn gì cả, giữ lại ảnh cũ
            $productModel = new SanPham();
