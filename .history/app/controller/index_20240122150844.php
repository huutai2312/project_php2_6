...
class Controller
{
    ...
    public function addToCart()
    {
        if (isset($_POST['addToCart'])) {
            ...
            if ($productId) {
                $product = $sanPhamModel->getProductById($product_id);

                if ($product) {
                    $cartIndex = $this->model->checkCart($productId);

                    if ($cartIndex != -1) {
                        $_SESSION['cart'][$cartIndex][1]++;
                    } else {
                        $_SESSION['cart'][] = [$productId, 1, $product['price'], $product['name']];
                    }
                }
            }
        }

        // Chuyển hướng hoặc làm các xử lý khác tùy theo yêu cầu của bạn
        // ...

        // Ví dụ: chuyển hướng về trang sản phẩm sau khi thêm vào giỏ hàng
        header('Location: /san-pham?id=' . $productId);
        exit();
    }
